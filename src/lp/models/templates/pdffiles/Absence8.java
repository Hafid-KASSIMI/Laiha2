...
import lp.models.templates.pdffiles.metrics.Absence8Metrics;
import lp.Settings;
...
public class Absence8 extends AbsenceListPDF {
    
    public Absence8() {
        super();
        pageMetrics = new Absence8Metrics();
    ...
    protected void add1PageGroup(Group g, PDPage page, int start, int stuCount, WEEK week, String pager) {
        ...
        pdf.placeString(((Absence8Metrics) pageMetrics).getBureau(), page, Settings.PREF_TEMPORARY.get(pageMetrics.getTemplate() + "_SURVEILANCE_DESKTOP")
                                + Settings.PREF_TEMPORARY.get(pageMetrics.getTemplate() + "_SURVEILANCE_NUMERO"));
        pdf.placeString(((Absence8Metrics) pageMetrics).getToAdmin(), page, Settings.PREF_TEMPORARY.get(pageMetrics.getTemplate() + "_TO_ADMINISTRATION"));
        pdf.placeString(((Absence8Metrics) pageMetrics).getSignature(), page, Settings.PREF_TEMPORARY.get(pageMetrics.getTemplate() + "_SIGNATURE"));
