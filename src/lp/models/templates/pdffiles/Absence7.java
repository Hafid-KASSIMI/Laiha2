...
import lp.models.templates.pdffiles.metrics.Absence7Metrics;
import lp.Settings;
...
public class Absence7 extends AbsenceListPDF {
    
    public Absence7() {
        super();
        pageMetrics = new Absence7Metrics();
    ...
    protected void add1PageGroup(Group g, PDPage page, int start, int stuCount, WEEK week, String pager) {
        ...
        pdf.placeString(((Absence7Metrics) pageMetrics).getBureau(), page, Settings.PREF_TEMPORARY.get(pageMetrics.getTemplate() + "_SURVEILANCE_DESKTOP")
                                + Settings.PREF_TEMPORARY.get(pageMetrics.getTemplate() + "_SURVEILANCE_NUMERO"));
        pdf.placeString(((Absence7Metrics) pageMetrics).getToAdmin(), page, Settings.PREF_TEMPORARY.get(pageMetrics.getTemplate() + "_TO_ADMINISTRATION"));
        pdf.placeString(((Absence7Metrics) pageMetrics).getSignature(), page, Settings.PREF_TEMPORARY.get(pageMetrics.getTemplate() + "_SIGNATURE"));
