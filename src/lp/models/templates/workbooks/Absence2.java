...
import lp.Settings;
import lp.models.templates.workbooks.metrics.Absence2Metrics;
 ...
 * @author H. KASSIMI
 */
public class Absence2 extends AbsenceWorkbook {
    
    private final String separator = "\n";
    
    public Absence2() {
        super();
        sheetMetrics = new Absence2Metrics();
        initializeTemplate();
    }
    ...
    protected void add1PageGroup(Group g, XSSFSheet sht, WEEK week) {
        ...
        sht.getRow(sheetMetrics.getMessage().getY()).getCell(sheetMetrics.getMessage().getX(), Row.MissingCellPolicy.CREATE_NULL_AS_BLANK).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_MESSAGE"));
        sheetMetrics.getMessage().doMerge(sht);
        for ( int i = sheetMetrics.getMessage().getY(), 
                n = sheetMetrics.getMessage().getY() + sheetMetrics.getMessage().getRowSpan() + 1; i < n; i++ ) {
            sht.getRow(i).getCell(sheetMetrics.getMessage().getX(), Row.MissingCellPolicy.CREATE_NULL_AS_BLANK).setCellStyle(sheetMetrics.getMessage().getStyle(sht.getWorkbook()));
        }
    }
