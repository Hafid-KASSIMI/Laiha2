...
import lp.Settings;
import lp.models.templates.workbooks.metrics.Absence6Metrics;
...
public class Absence6 extends AbsenceListWorkbook {
    private final String separator = "\n";
    
    public Absence6(){
        super();
        sheetMetrics = new Absence6Metrics();
        initialize();
    }

    @Override
    protected void add1PageGroup(Group g, XSSFSheet sheet, int start, int stuCount) {
        super.add1PageGroupHoleWeek(g, sheet, start, stuCount);
    }
    ...
    protected void add1PageGroup(Group g, XSSFSheet sht, int start, int stuCount, WEEK week, String page) {
        ...
        sht.getRow(((Absence6Metrics) sheetMetrics).getBureau().getY()).getCell(((Absence6Metrics) sheetMetrics).getBureau().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_SURVEILANCE_DESKTOP")
                                + Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_SURVEILANCE_NUMERO"));
        sht.getRow(sht.getLastRowNum() - 1).getCell(((Absence6Metrics) sheetMetrics).getSignature().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_SIGNATURE"));
        sht.getRow(sht.getLastRowNum()).getCell(((Absence6Metrics) sheetMetrics).getToAdmin().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_TO_ADMINISTRATION"));
    ...
    protected void placeStudent(int index, XSSFSheet sht, Student stu) {
        ...
        current.getCell(row.getAdditional().getX()).setCellValue(stu.getNum());
