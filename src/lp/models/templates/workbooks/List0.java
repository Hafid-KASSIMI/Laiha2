...
import lp.Settings;
import lp.models.templates.workbooks.metrics.List0Metrics;
...
public class List0 extends ListWorkbook {
    
    private final String separator;
    
    public List0(){
        super();
        sheetMetrics = new List0Metrics();
    ...
    protected void add1PageGroup(Group g, XSSFSheet sht, int start, int stuCount) {
        ...
        sht.getRow(((List0Metrics) sheetMetrics).getCustomLabel1().getY()).getCell(((List0Metrics) sheetMetrics).getCustomLabel1().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_CUSTOM_STRING"));
        sht.getRow(((List0Metrics) sheetMetrics).getCustomLabel2().getY()).getCell(((List0Metrics) sheetMetrics).getCustomLabel2().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_CUSTOM_STRING2"));
        sht.getRow(((List0Metrics) sheetMetrics).getCustomLabel3().getY()).getCell(((List0Metrics) sheetMetrics).getCustomLabel3().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_CUSTOM_STRING3"));
    }
