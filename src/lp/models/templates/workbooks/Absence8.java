...
import lp.Settings;
import lp.models.templates.workbooks.metrics.Absence8Metrics;
...
public class Absence8 extends AbsenceListWorkbook {
    
    private final String separator = "\n";
    
    public Absence8(){
        super();
        sheetMetrics = new Absence8Metrics();
        initialize();
    }
    ...
    protected void add1PageGroup(Group g, XSSFSheet sht, int start, int stuCount, WEEK week, String page) {
        ...
        sht.getRow(((Absence8Metrics) sheetMetrics).getBureau().getY()).getCell(((Absence8Metrics) sheetMetrics).getBureau().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_SURVEILANCE_DESKTOP")
                                + Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_SURVEILANCE_NUMERO"));
        sht.getRow(sht.getLastRowNum() - 1).getCell(((Absence8Metrics) sheetMetrics).getSignature().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_SIGNATURE"));
        sht.getRow(sht.getLastRowNum()).getCell(((Absence8Metrics) sheetMetrics).getToAdmin().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_TO_ADMINISTRATION"));
