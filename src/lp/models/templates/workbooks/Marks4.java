...
import lp.Settings;
import lp.models.templates.workbooks.metrics.Marks4Metrics;
...
public class Marks4 extends MarksWorkbook {
    
    public Marks4() {
        super();
        sheetMetrics = new Marks4Metrics();
    ...
    protected void add1PageGroupNoMarks(Group group, XSSFSheet sheet, int start, int stuCount) {
        sheet.getRow(((Marks4Metrics) sheetMetrics).getTeacher().getY()).getCell(((Marks4Metrics) sheetMetrics).getTeacher().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_EMPTY_STRING"));
        sheet.getRow(((Marks4Metrics) sheetMetrics).getMatter().getY()).getCell(((Marks4Metrics) sheetMetrics).getMatter().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_EMPTY_STRING"));
        sheet.getRow(((Marks4Metrics) sheetMetrics).getSemester().getY()).getCell(((Marks4Metrics) sheetMetrics).getSemester().getX()).setCellValue(Settings.PREF_TEMPORARY.get(sheetMetrics.getTemplate() + "_EMPTY_STRING"));
    ...
    protected void add1PageGroupWithMarks(Group group, XSSFSheet sheet, int start, int stuCount, 
        ...
        sheet.getRow(((Marks4Metrics) sheetMetrics).getTeacher().getY()).getCell(((Marks4Metrics) sheetMetrics).getTeacher().getX()).setCellValue(group.getTeacher(matter.getCode()));
        sheet.getRow(((Marks4Metrics) sheetMetrics).getMatter().getY()).getCell(((Marks4Metrics) sheetMetrics).getMatter().getX()).setCellValue(matter.getLabel());
        sheet.getRow(((Marks4Metrics) sheetMetrics).getSemester().getY()).getCell(((Marks4Metrics) sheetMetrics).getSemester().getX()).setCellValue(semester.intValue());
        placeCommonHeader(group, sheet);
    ...
    private void placeCommonHeader(Group group, XSSFSheet sheet) {
        sheet.getRow(((Marks4Metrics) sheetMetrics).getKingdom().getY()).getCell(((Marks4Metrics) sheetMetrics).getKingdom().getX()).setCellValue(Settings.PREF_TEMPORARY.get("KINGDOM-AR").replaceAll("%nl%", "\n"));
        ...
        sheet.getRow(((Marks4Metrics) sheetMetrics).getTeacherLbl().getY()).getCell(((Marks4Metrics) sheetMetrics).getTeacherLbl().getX()).setCellValue(Settings.PREF_TEMPORARY.get("MARKS_TEACHER_LABEL"));
        sheet.getRow(((Marks4Metrics) sheetMetrics).getMatterLbl().getY()).getCell(((Marks4Metrics) sheetMetrics).getMatterLbl().getX()).setCellValue(Settings.PREF_TEMPORARY.get("MARKS_MATTER_LABEL"));
        sheet.getRow(((Marks4Metrics) sheetMetrics).getSemesterLbl().getY()).getCell(((Marks4Metrics) sheetMetrics).getSemesterLbl().getX()).setCellValue(Settings.PREF_TEMPORARY.get("MARKS_SEMESTER_LABEL"));
        sheet.getRow(((Marks4Metrics) sheetMetrics).getYearLbl().getY()).getCell(((Marks4Metrics) sheetMetrics).getYearLbl().getX()).setCellValue(Settings.PREF_TEMPORARY.get("MARKS_YEAR_LABEL"));
    }
